...
document.getElementById("mega-clear-plan")?.addEventListener("click",()=>{const account=megaNormalizeAccount(); megaWeekDays(megaPlanWeekOffset).forEach((date)=>["Petit-déjeuner","Déjeuner","Dîner"].forEach((slot)=>delete account.mealPlan[megaPlanKey(date,slot)])); sauvegarderEtatApplication();megaRenderPlanner();megaRenderShopping();});
document.getElementById("mega-auto-plan")?.addEventListener("click",()=>{const account=megaNormalizeAccount(); megaWeekDays(megaPlanWeekOffset).forEach((date,di)=>{["Petit-déjeuner","Déjeuner","Dîner"].forEach((slot,si)=>{const options=megaRecipesForSlot(slot); if(options.length){const idx=Math.abs((date.getDate()*7+di*3+si*11))%options.length; account.mealPlan[megaPlanKey(date,slot)]={recipeName:options[idx].nom,portions:1};}});});sauvegarderEtatApplication();megaRenderPlanner();megaRenderShopping();});
